% Optimal Resource Allocation and Scheduling of Athletic Activities -- defence slides.
%
...
\ifdefined\shownotes
  ...
  \providecommand{\notesfont}{Helvetica}
  \newfontfamily\gr{\notesfont}
  % speaker-notes.tex supplies both the notes and how they are emitted, so
  % nothing about them ever requires editing this file.
...
\date{21 September 2026}   % defence date, fixed so it does not follow the build day
...
% ---------------------------------------------------------------- data
\begin{frame}{Real data: 32 months of my own training}
\begin{columns}
\column{.55\textwidth}
\begin{itemize}\itemsep6pt
  ...
  \item Jun 2026: CTL $78.1$, ATL $91.5$, \textbf{TSB $-13.4$}
\end{itemize}
\column{.42\textwidth}
\centering\small
\begin{tabular}{lrrr}
...
\textbf{17 candidate races}, binary $x_i \in \{0,1\}$
\vspace{-4pt}
\[
\max \sum_i \underbrace{q_i}_{\substack{\text{P(not ruined}\\ \text{by weather)}}}
\bigl(\,1\cdot\text{pts}_i + 3\cdot\text{strat}_i + 2\cdot\text{enj}_i
+ 2\cdot\text{fit}_i\,\bigr)\, x_i
\]
\vspace{-16pt}
\begin{center}\small
$\text{enj}_i = 0.3\,\text{org} + 0.3\,\text{scenery} + 0.4\,\text{swim}$\\[3pt]
$\text{fit}_i = \tfrac12 f(\text{bike}) + \tfrac12 f(\text{run})$,\quad
$f = 10\bigl(1 - \min(g,15)/15\bigr)$,\quad $g = $ metres climbed per km
\end{center}
\vspace{-2pt}
\begin{itemize}\itemsep5pt
  ...
  \item \textbf{readiness gate from Model 1:} projected CTL must reach the class minimum
\end{itemize}
\end{frame}
...
\begin{frame}{Model 3 --- the LP derives ``steady bike, hard run''}
\centering
...
\begin{itemize}\itemsep5pt
  ...
  \item binding constraint \textbf{switches with distance}: speed caps $\to$ energy
        $\to$ infeasible without fuelling
  \item the 70.3 run: my \textbf{29-minute} wall is the energy constraint being violated in real life
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep10pt
  ...
  \item Spanning the syllabus: LP \& Simplex, duality \& sensitivity,
        IP with Branch \& Bound, network flows \& total unimodularity
